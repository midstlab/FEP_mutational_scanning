...
def config_gen(pdb_name):
	...
	f = open(str(file_name_wh_ex)+"_config_min_forw.conf", 'w')
	f.write("""
...
""")
	...
	f.write("%-10s\t\t\t\t%-10s\n" % ("bincoordinates",str(file_name_wh_ex)+"_min.coor"))
	f.write("%-10s\t\t\t\t%-10s\n" % ("binvelocities",str(file_name_wh_ex)+"_min.vel"))
	f.write("%-10s\t\t\t\t%-10s\n" % ("extendedsystem",str(file_name_wh_ex)+"_min.xsc"))
	f.write("\n")
	f.write("\n")
	f.write("\n")
	f.write("""
...
dcdfreq                 10000
""")
	f.write("DCDfile     			%s_min_forw.dcd\n" % (str(file_name_wh_ex)))

	f.write("\n")
	f.write("\n")

	f.write("outputname     		%s_min_forw\n" % (str(file_name_wh_ex)))
	f.write("restartname    		%s_min_forw\n" % (str(file_name_wh_ex)))
	f.write("\n")
	f.write("\n")
	f.write("""
binaryoutput            yes
...
""")
	...
	f.write("alchCol                 B\n")
	f.write("alchOutFile             %s_forw.fepout\n" % (str(file_name_wh_ex)))
...
runFEP 0.0 1.0 0.03125 $numSteps
""")
